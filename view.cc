...
namespace L3DPP
{
    ...
    View::View(const unsigned int id, L3DPP::DataArray<float4>* lines,
               ...
               Eigen::Vector3d ROI[4], Eigen::Vector3d* centrals, int line_num,
               L3DPP::DataArray<float>* superpixels) :
        id_(id), lines_(lines), K_(K), R_(R), t_(t),
        width_(width), height_(height), superpixels_(superpixels)
    {
        // init
        ...
        // central of lines_
        //todo::line_num redundant, can get from lines_width_
        for (int i = 0; i < line_num; ++i)
        {
            centrals_.push_back(centrals[i]);
        }

        // camera
        pp_ = Eigen::Vector3d(K_(0,2),K_(1,2),1.0);  //intrinsic K offset cx, cy
